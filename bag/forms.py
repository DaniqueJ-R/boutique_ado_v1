...
def add_to_bag(request, item_id):
    """Add and update quantity of items in shopping bag."""
    ...
    if size:
        if item_id in list(bag.keys()):
            if size in bag[item_id]['item_by_size'].keys():
                ...
                messages.success(request, f'Updated {size.upper()} {product.name} to {bag[item_id]['item_by_size'][size]}!')
            else:
                bag[item_id]['item_by_size'][size] = quantity
        ...
        if item_id in list(bag.keys()):  # if no size
...
def adjust_bag(request, item_id):
    """Adjusts item quantity in shopping bag."""
